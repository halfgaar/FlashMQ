 ...
 */
int full_stoi(const std::string &key, const std::string &value, int min=std::numeric_limits<int>::min(), int max=std::numeric_limits<int>::max())
{
    size_t ptr;
    ...
    if (newVal < min || newVal > max)
    {
        ...
        oss << "Value '" << value << "' out of range for '" << key << "', which must be between " << min << " and " << max;
    ...
    for (std::string &line : lines)
    {
        ...
        try
        {
            ...
            else if (curParseLevel.top() == ConfigParseLevel::BridgeLazySubscriptions)
            {
                ...
                {
                    curLazySubscription.value().qos = full_stoi(key, valueTrimmed, 0, 2);
                }
