...
void queueSendSubAckInOriginatingClient(const std::optional<SubAckReleaseTrigger> &t)
{
    if (!t)
        return;

    if (!t->m_client_thread)
        return;

    t->m_client_thread->queueImmediateStagedSuback(t->m_client, t->m_staged_suback_packet_id);
}
...
void TrackedSubscriptionState::assumePendingPacketIdsLost(Session *session)
{
    if (!session)
        return;

    if (inFlightTrackedSubscriptions)
        session->increaseFlowControlQuotaLocked(inFlightTrackedSubscriptions.value().id);

    if (inFlightTrackedUnsubscriptions)
        session->increaseFlowControlQuotaLocked(inFlightTrackedUnsubscriptions.value().id);
}
    ...
    const std::shared_ptr<BridgeState> &bridgeState, const ProcessTrackedSubscriptionMutationsModifier modifier)
{
    ...
    {
        assumePendingPacketIdsLost(session.get());
    ...
    if (curPosResending != trackedSubscriptions->end())
    {
        ...
        while (curPosResending != trackedSubscriptions->end() && cur_packet_size <= packet_size_limit && batch_entry <= batch_size)
        {
            ...
            cur_packet_size += 3 + t.pattern.size();
 ...
 *
 * When processing mutations, we don't actively send UNSUBSCRIBE packets (also as documented in
 * processTrackedSubscriptionMutations). It can happen that clients keep subscribing and unsubscribing to patterns,
 * so we only send the UNSUBSCRIBE packets when the subscription has been out of use long enough.
 */
    ...
    const std::shared_ptr<BridgeState> &bridgeState, const PurgeTrackedSubscriptionModifier modifier)
{
    ...
    while (this->curPosPurging != this->trackedSubscriptions->end() && cur_packet_size < max_packet_size && batch_entry < batch_size)
    {
        ...
        purgeCount++;
        cur_packet_size += tracked_subscr_pos->val.pattern.size();
        ...
        if (tracked_subscr_pos->val.empty())
        {
            ...
            if (Logger::getInstance()->wouldLog(LOG_UNSUBSCRIBE))
            {
                Logger::getInstance()->log(LOG_UNSUBSCRIBE)
                    << "Unsubscribing from  '" << filter
...

    {
        ...
        log << "Bridge " << network_client->getClientId() << " unsubscribing stale lazy subscriptions: checked "
            << this->purgeCount << " of " << this->purgeTotal << " tracked subscriptions.";
...
{
    auto all_triggers = outgoingSubscriptionsToSubackReleases.equal_range(id);

    for (auto i = all_triggers.first; i != all_triggers.second; )
    {
        auto pos = i++;
