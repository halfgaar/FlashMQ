...
void MainApp::start()
{
    ...
    if (getFuzzMode())
    {
        ...
        try
        {
            ...
            std::shared_ptr<Client> client = std::make_shared<Client>(fd, threaddata, nullptr, connectionProtocol, false, nullptr, settings, true);
            std::shared_ptr<Client> subscriber = std::make_shared<Client>(fdnull, threaddata, nullptr, connectionProtocol, false, nullptr, settings, true);
            subscriber->setClientProperties(ProtocolVersion::Mqtt311, "subscriber", "subuser", true, 60);
            subscriber->setAuthenticated(true);

            std::shared_ptr<Client> websocketsubscriber = std::make_shared<Client>(fdnull2, threaddata, nullptr, ConnectionProtocol::WebsocketMqtt, false, nullptr, settings, true);
        ...
        for (int i = 0; i < num_fds; i++)
        {
            ...
            try
            {
                if (cur_fd != taskEventFd)
                {
                    ...
                    std::shared_ptr<Client> client = std::make_shared<Client>(fd, thread_data, clientSSL, listener->connectionProtocol.value(), listener->isHaProxy(), addr, settings);
