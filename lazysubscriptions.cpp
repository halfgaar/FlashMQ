    ...
    const std::vector<std::string> &subtopics, const uint8_t qos)
{
    ...
    for(ReceivingLazySubscriber &receiver : collected_receivers)
    {
        ...
        const bool doWakeup = bridgeState->addTrackedSubscriptionMutation(
            TrackedSubscriptionMutation(pattern, effective_qos, originating_session->getClientId(), originating_session, task));

        // TODO: we have to defer this when we're still working on sending our tracked list after a reconnection.
        if (doWakeup)
        {
            receiver.thread->queueProcessTrackedSubscriptionMutations(bridgeState);
        }
    }
}
