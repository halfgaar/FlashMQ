...
SessionsAndSubscriptionsResult SessionsAndSubscriptionsDB::readDataV3V4V5V6V7()
{
    ...
    while (!feof(f))
    {
        ...
        std::shared_ptr<Client> dummyClient(std::make_shared<Client>(ClientType::Normal, -1, dummyThreadData, FmqSsl(), ConnectionProtocol::Mqtt, false, nullptr, settings, false));
