...
void MainTests::testSavingSessions()
{
    try
    {
        ...
        std::shared_ptr<Client> c1(new Client(ClientType::Normal, -1, t, FmqSsl(), ConnectionProtocol::Mqtt, false, nullptr, settings, false));
        ...
        std::shared_ptr<Client> c2(new Client(ClientType::Normal, -1, t, FmqSsl(), ConnectionProtocol::Mqtt, false, nullptr, settings, false));
        ...
        const auto publish_age = queuedPublishLoaded->getPublish().getAge<std::chrono::milliseconds>().count();
        FMQ_VERIFY(publish_age > 900);
        FMQ_VERIFY(publish_age < 2100);
...
void MainTests::testParsePacketHelper(const std::string &topic, uint8_t from_qos, bool retain)
{
    ...
    std::shared_ptr<Client> dummyClient(new Client(ClientType::Normal, -1, t, FmqSsl(), ConnectionProtocol::Mqtt, false, nullptr, settings, false));
...
void MainTests::testTopicMatchingInSubscriptionTreeHelper(const std::string &subscribe_topic, const std::string &publish_topic, int match_count)
{
    ...
    std::shared_ptr<Client> client = std::make_shared<Client>(ClientType::Normal, -1, td, FmqSsl(), ConnectionProtocol::Mqtt, false, nullptr, *settings, false);
